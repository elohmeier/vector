...
fn default_max_event_age_seconds() -> u64 {
    DEFAULT_MAX_EVENT_AGE_SECS
}

fn default_dedupe_retention_seconds() -> u64 {
    DEFAULT_DEDUPE_RETENTION_SECS
}

fn default_watch_timeout_seconds() -> u32 {
...
impl KubernetesEventsSource {
    ...
    async fn process_apply_event(
        ...
        if let Some(ns) = namespace.or_else(|| event.metadata.namespace.as_deref()) {
    ...
    fn kind_allowed(&self, event: &KubeEvent) -> bool {
        match (&self.kind_filter, &event.regarding) {
            (None, _) => true,
            (Some(filter), Some(reference)) => reference
                .kind
                .as_ref()
                .map_or(false, |k| filter.contains(k)),
            (Some(_), None) => false,
        }
    }
...
enum DedupResult {
    Added,
    Updated { previous: Option<KubeEvent> },
    Duplicate,
}
            ...
            Some(entry) => {
                match compare_resource_versions(&resource_version, &entry.resource_version) {
                    ...
                    std::cmp::Ordering::Greater => {
                        let previous = include_previous.then(|| entry.event.clone());
